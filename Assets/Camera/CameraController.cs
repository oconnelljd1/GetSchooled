...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField]
    private float maxFollowDist = 10f, minFollowDist = 5f, moveSpeed = 1f, rotSpeed = 90f;
    ...
    void Update()
    {
        if(Input.GetAxis("Mouse X") != 0)
        {
            ...
            transform.RotateAround(target.position, Vector3.up, Input.GetAxis("Mouse X") * rotSpeed * Time.deltaTime);
            lastRotation = transform.eulerAngles.y;
        }
        transform.RotateAround(target.position, transform.right, -Input.GetAxis("Mouse Y") * rotSpeed * Time.deltaTime);
